...
def get_dataset_root():
    ...
    folder = filedialog.askdirectory(title="Select YOLO Root")
    root.destroy()
    return folder
    ...
    with open(label_path, 'r') as f:
        for line in f.readlines():
            ...
            if len(parts) >= 9: # YOLO OBB
                ...
                # Optional: draw a solid border on the overlay too
                ...
                cv2.rectangle(overlay, p1, p2, (0, 255, 0), -1) # -1 fills the rectangle

    # Blend the overlay with the original image
    # img2 = (original * (1 - alpha)) + (overlay * alpha)
...
def main():
    ...
    img_dir, lbl_dir = Path(root_path)/"images/train", Path(root_path)/"labels/train"
    ...
    while True:
        ...
        lbl_path = os.path.join(lbl_dir, img_files[idx].rsplit('.', 1)[0] + ".txt")
        
        raw_img = cv2.imread(img_path)
        if raw_img is None: 
            idx = (idx + 1) % len(img_files); continue
        
        labeled_img = draw_labels(raw_img.copy(), lbl_path)
        h, w, _ = labeled_img.shape

        # Pre-calculate stitching geometry to save CPU
        # Combined width is original width + space for the square zoom (height x height)
        combined_w = w + h 
        ...
        while True:
            ...
            cv2.putText(zoom_pane, "ZOOM", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
            ...
            # We do this in one go to reduce lag
            ...
            key = cv2.waitKey(15) # Shorter wait for snappier feel
            if key != -1:
                k = key & 0xFF
                if k == ord('q') or k == 27: return
                if k == ord('d') or key in [2555904, 83, 3]: 
                    idx = (idx + 1) % len(img_files); break
